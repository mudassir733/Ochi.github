...
function Navbar() {
  ...
  const [isTransparent, setIsTransparent] = useState(true); // Added initial state for transparency
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      setIsTransparent(window.scrollY <= scrollThreshold);
    };

    window.addEventListener("scroll", handleScroll);

    // Initial call to set states on component mount
    handleScroll();
  ...
  return (
    <div
      className={`w-full px-12 py-4 duration-200 font-['Neue_Montreal'] flex justify-between items-center fixed z-[900] font-normal ${
        isTransparent ? "bg-transparent" : "backdrop-blur-sm py-3"
      }`}
    >
      ...
      <nav className="links flex gap-10 items-center py-0">
      ...
      <div className="menu-div w-20 lg:hidden flex items-center justify-center md:visible">
        <MotionConfig transition={{ duration: 0.3, ease: "easeInOut" }}>
          ...
          >
            <motion.span
              ...
              className="absolute w-7 h-[1px] bg-white"
              variants={{
                open: {
